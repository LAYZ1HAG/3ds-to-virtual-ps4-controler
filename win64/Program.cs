...
namespace YaPiDoor
{
    class Program
    {
        ...
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "YaPiDoor_" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(tempDir);
            string dllPath = Path.Combine(tempDir, "ViGEmClient.dll");

            var asm = Assembly.GetExecutingAssembly();
            string resourceName = "YaPiDoor.ViGEmClient.dll";

            using (var resStream = asm.GetManifestResourceStream(resourceName))
            {
                if (resStream == null)
                {
                    // На случай если фактическое имя ресурса отличается - ищем подходящее
                    foreach (var name in asm.GetManifestResourceNames())
                    {
                        if (name.EndsWith("ViGEmClient.dll", StringComparison.OrdinalIgnoreCase))
                        {
                            resourceName = name;
                            break;
                        }
                    }
                    using (var resStream2 = asm.GetManifestResourceStream(resourceName))
                    using (var fileStream2 = File.Create(dllPath))
                    {
                        resStream2!.CopyTo(fileStream2);
                    }
                }
                else
                {
        ...
        static int Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"[ERR] Failed to extract ViGEmClient.dll: {ex.Message}");
                Pause();
                return 1;
            }
                ...
                catch (SocketException ex)
                {
                    if (ex.SocketErrorCode == SocketError.TimedOut)
                    {
                        if (_connectionStatus == "Connected")
                        {
                            ...
                            Console.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [DEBUG] Data delay! Packet loss for {(DateTime.UtcNow - lastPacketTime).TotalMilliseconds} мс");
                            Console.ResetColor();
                        }
                        ...
                        if (!_isMenuInteracting && _connectionStatus == "Connected" && (DateTime.UtcNow - lastPacketTime).TotalSeconds >= 3.0)
                        {
                            ...
                            Console.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [NET] Connection lost (no data for 3 sec).");
                            Console.ResetColor();
